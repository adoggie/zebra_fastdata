...
class TradeType(object):
    ...
    SPOT = (1,'spot')
    SWAP = (2,'swap')
    SPOTC = (3, 'spotc') # base coin
    SWAPC = (4, 'swapc') # base  coin
    INDEX = (5, 'index') # 指数
    MASTER = (6,'master') # 主力合约
...
class RapidMessage(object):
    ...
    def __init__(self):
        ...
        self.datetime = int(datetime.datetime.now().timestamp()*1000)  # utc timestamp
...
class KLine(RapidMessage):
    ...
    def rand_one():
        ...
        kline.datetime = int(datetime.datetime.now().timestamp()*1000)
...
class MxAlive(RapidMessage):
    Type = MessageType.MxAlive[1]
    def __init__(self):
        RapidMessage.__init__(self)
        self.exchange = ExchangeType.Undefined[1]
        self.tt = TradeType.Undefined[1]
