...
class RegistryClient(object):
  ...
  def open(self):
    ...
    for topic in self.cfgs.get("registry_client.subscribe",[]):
      self.sock_s.setsockopt(zmq.SUBSCRIBE, topic.encode())

    if self.cfgs.get('ha_enable',1) and self.cfgs.get('ha_role','master') == 'slave':
      topic = self.cfgs.get('ha_master_keepalive_listen')
      self.sock_s.setsockopt(zmq.SUBSCRIBE, topic.encode())
  ...
  def onServiceQuery(self,req):
    ...
    resp.start = int( self.start.timestamp()*1000)
    resp.pid = os.getpid()
    resp.now = int( datetime.datetime.now().timestamp()* 1000)
  ...
  def keep_alive(self,**kvs):
    ...
    resp.now = int( datetime.datetime.now().timestamp()*1000)
    resp.params = kvs
    resp.ip = self.cfgs.get('ip','')
    resp.tag = self.cfgs.get('tag','')
    self.send_message(resp)
    # text = resp.marshall()
    # self.sock_p.send(text.encode())
